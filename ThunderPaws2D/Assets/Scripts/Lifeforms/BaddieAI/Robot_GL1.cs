...
public class Robot_GL1 : DamageableLifeform {
    ...
    private void CheckForHorizontalEquality(float dirToTarget) {
        ...
        Debug.DrawRay(transform.position, (_facingRight ? Vector2.right : Vector2.left) * _visionRaylength, Color.red);

        RaycastHit2D horizontalCheck = Physics2D.Raycast(transform.position, _facingRight ? Vector2.right : Vector2.left, _visionRaylength, targetLayer);
